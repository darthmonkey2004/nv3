...
class screencap():
	def __init__(self, camera_id=3, width=None, height=None):
		if width is None:
			self.width = 1024
		else:
			self.width = width
	...
	def read(self):
		img = self.img
		self.img = imutils.resize(img, width=self.width, height=self.height)
		if img is None:
			return False, None
		else:
			return True, img
...
class capture():
	...
	def run(self):
		log(f"capture.init():Capture starting...", 'info')
		self.run_loop = True
