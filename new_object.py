...
from cv2 import LINE_AA as line_type
import os
log = nv_logger().log_msg
allvars = ['data', 'camera_id', 'box', 'class_name', 'priority', 'img', 'confidence', 'tolerance', 'distance']
for var in allvars:
	if var not in globals().keys():
		globals()[var] = None

class new_object():
	def __init__(self, camera_id: int, object_name: str):
		self.object_name = object_name
		self.camera_id = camera_id

	def new(self, **args):
		global box, class_name, priority, img, confidence, tolerance, distance
		for key in args.keys():
			val = args[key]
			globals()[key] = val
		if camera_id is not None:
			#if camera_id in args, override init value
			self.camera_id = camera_id
		if self.object_name == 'message':
			return message(camera_id=self.camera_id, priority=priority)
		elif self.object_name == 'nvbox' or self.object_name == 'box':
			if isinstance(box, nvbox):
				box = box.box
				if box.img is not None and img is None:
					img = box.img
			if type(box) != tuple:
				try:
					box = tuple(box)
				except Exception as e:
					log(f"message.new_object:Provided box is wrong type, unable to convert to tuple! type:{type(box)}, box:{box}", 'error')
					box = None
			return nvbox(camera_id=self.camera_id, class_name=class_name, box=box, img=img, confidence=confidence, tolerance=tolerance, distance=distance)
		else:
			log(f"Unknown object name provided: {self.object_name}!", 'error')
			return None
...
class nvbox(message):
	def __init__(self, camera_id, box, class_name='Detection', img=None, confidence=None, tolerance: float=0, distance: float=0):
		self.camera_id = camera_id
		self.conf = readConf()
		...
		if self.class_name in self.classes:
			if self.class_name == 'person':
				...
				if self.class_name not in self.animals and self.class_name not in self.vehicles:
					self.type = 'object'
				elif self.class_name in self.animals:
					self.type = 'animal'
				elif self.class_name in self.vehicles:
					self.type = 'vehicle'
		else:
			self.type = 'recognized_face'
