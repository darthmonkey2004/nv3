...
log(txt, 'info')
class tracker():
	def __init__(self, img, det, tracker_type=None, max_age: int=200):
		self.camera_id = int(det.camera_id)
		...
		det.age = 0
		det.success = False
		det.types = self.opts['tracker']['types']['available']
		self.oldbox = None
		...
		det.tracker = self.new_tracker(tracker_type=tracker_type)
